            ...
            <form onSubmit={handleSignUp}>
              <fieldset>
                ...
                <div className="relative">
                  <input type={togglePassword ? "text" : "password"} name="password" className="input w-full" placeholder="Password" />
                  <button onClick={() => setTogglePassword(!togglePassword)} type="button" className="btn absolute top-0 right-0 z-1">
                    {togglePassword ? <IoEyeOutline size={20} /> : <IoEyeOffOutline size={20} />}
                  </button>
                </div>

                <small className="text-error">{error}</small>
                <button className="btn bg-[#00c497] text-white mt-3 w-full" type="submit">Sign Up</button>
              </fieldset>
            </form>
